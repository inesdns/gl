...
def create_database():
    engine = create_engine('postgresql+psycopg2://postgres:postgres@host.docker.internal:1234/postgres')
...
def create_schema():
    engine = create_engine('postgresql+psycopg2://postgres:postgres@host.docker.internal:1234/test')
...
def stage_table():
    engine = create_engine('postgresql+psycopg2://airflow:airflow@host.docker.internal:5432/airflow')
    session = scoped_session(sessionmaker(bind=engine))
